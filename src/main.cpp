...
int main() {
    ...
    if (visualizeGif) {
        ...
        while (!validGifPath) {
            ...
            if (gifOutputPath.empty()) {
                ...
                fs::path inputPath(inputImagePath);
                fs::path gifFileName = inputPath.stem().string() + "_process.avi";
                ...
                if (gifPath.extension().empty()) {
                    gifOutputPath += ".avi";
                    ui.showInfo("Menambahkan ekstensi file: " + gifOutputPath);
                }
    ...
    try {
        ...
        if (visualizeGif) {
            ui.showLoading("Membuat visualisasi", 50);
            ...
            if (!gifSuccess) {
                ui.showWarning("Gagal membuat visualisasi. Melanjutkan...");
            } else {
                ui.showSuccess("Visualisasi berhasil disimpan.");
            }
        }
